-- ============================================================================
-- Invite-Chat — vollständiges Schema
-- ============================================================================
...
for insert to authenticated
with check (
  sender_id = (select auth.uid()) and public.is_member(conversation_id)
  and ciphertext is not null and iv is not null
);
...
begin
  ...
  key := case when me < other_user then me::text || ':' || other_user::text
  ...
  exception when unique_violation then
    select id into conv from public.conversations where dm_key = key; return conv;
  end;

-- Aufräumen alter Nachrichten, Versuche und Codes
...
begin
  ...
  delete from public.conversation_keys ck
   where ck.created_at < now() - make_interval(days => msg_days)
     and not exists (select 1 from public.messages m where m.conversation_id = ck.conversation_id and m.key_epoch = ck.epoch);
  ...
  update public.invite_codes set payload = null, payload_iv = null, payload_salt = null
   where payload is not null and (used_at is not null or created_at < now() - make_interval(mins => dev_min));
  delete from public.invite_codes where kind = 'invite' and used_at is null and expires_at is not null
   and expires_at < now() - make_interval(days => code_days);
  delete from public.conversations c where c.last_message_at < now() - make_interval(days => msg_days)

...
create or replace function public.is_member(conv uuid) returns boolean language sql security definer stable set search_path=public,pg_temp as $$
  select exists(select 1 from public.conversation_members where conversation_id=conv and user_id=(select auth.uid()));
$$;
create or replace function public.is_owner(conv uuid) returns boolean language sql security definer stable set search_path=public,pg_temp as $$
  select exists(select 1 from public.conversation_members where conversation_id=conv and user_id=(select auth.uid()) and role='owner');
$$;
...
create or replace function public.touch_conversation() returns trigger language plpgsql security definer set search_path=public,pg_temp as $$
...
create policy "profiles_set_key_once" on public.profiles for update to authenticated
...
create policy "conversations_rename_by_owner" on public.conversations for update to authenticated using (type='group' and public.is_owner(id)) with check (type='group' and public.is_owner(id));
...
create policy "members_leave" on public.conversation_members for delete to authenticated using (user_id=(select auth.uid()));
create policy "conv_keys_read" on public.conversation_keys for select to authenticated using (public.is_member(conversation_id));
create policy "conv_keys_write" on public.conversation_keys for insert to authenticated with check (public.is_member(conversation_id) and created_by=(select auth.uid()));
create policy "messages_read" on public.messages for select to authenticated using (public.is_member(conversation_id));
create policy "messages_write" on public.messages for insert to authenticated with check (sender_id=(select auth.uid()) and public.is_member(conversation_id) and ciphertext is not null and iv is not null);
create policy "messages_delete_own" on public.messages for delete to authenticated using (sender_id=(select auth.uid()));

create or replace function public.start_dm(other_user uuid) returns uuid language plpgsql security definer set search_path=public,pg_temp as $$
declare me uuid:=(select auth.uid()); key text; conv uuid;
begin
 ...
 exception when unique_violation then select id into conv from public.conversations where dm_key=key; return conv;
 end;
 insert into public.conversation_members(conversation_id,user_id,role) values(conv,me,'owner'),(conv,other_user,'member');
 return conv;
end;
$$;

create or replace function public.create_group(p_title text,p_members uuid[]) returns uuid language plpgsql security definer set search_path=public,pg_temp as $$
declare me uuid:=(select auth.uid()); conv uuid; t text:=nullif(btrim(coalesce(p_title,'')),'');
begin
 if me is null then raise exception 'Nicht angemeldet' using errcode='42501'; end if;
 ...
 insert into public.conversation_members(conversation_id,user_id,role) select conv,p.id,'member' from public.profiles p where p.id=any(p_members) and p.id<>me on conflict do nothing;
 return conv;
end;
$$;

create or replace function public.invite_to_group(p_conversation uuid,p_user uuid) returns void language plpgsql security definer set search_path=public,pg_temp as $$
begin
...
create or replace function public.rotate_conversation_key(p_conversation uuid) returns int language plpgsql security definer set search_path=public,pg_temp as $$
declare e int;
begin

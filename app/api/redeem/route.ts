...
export async function POST(request:NextRequest){
 ...
 const admin=createAdminClient(),ipHash=clientIpHash(request,pepper);if(await throttled(admin,ipHash))return NextResponse.json({error:`Zu viele Versuche. Bitte in ${WINDOW_MINUTES} Minuten noch einmal.`},{status:429})
 const now=new Date().toISOString(),{data:claimed}=await admin.from('invite_codes').update({used_at:now}).eq('code_hash',hashCode(auth,pepper)).is('used_at',null).or(`expires_at.is.null,expires_at.gt.${now}`).select('id,kind,for_user,payload,payload_iv,payload_salt').maybeSingle()
 ...
 const email=`${crypto.randomUUID()}@${process.env.INVITE_EMAIL_DOMAIN||'invite.local'}`;const {data:created,error}=await admin.auth.admin.createUser({email,email_confirm:true,password:crypto.randomBytes(48).toString('base64url')})

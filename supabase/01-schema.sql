...
-- security definer, sonst prueft die Policy auf conversation_members sich
-- selbst und Postgres bricht mit "infinite recursion" ab.
...
-- invite_codes und redeem_attempts bekommen BEWUSST keine Policy.
-- Damit kommt weder anon noch authenticated jemals an sie heran; nur der
-- Secret Key auf dem Server, der RLS umgeht, darf lesen und schreiben.
...
-- Der oeffentliche Identitaetsschluessel darf genau einmal gesetzt werden.
-- Danach ist die Profilzeile eingefroren: selbst wer eine Sitzung uebernimmt,
-- kann keinen eigenen Schluessel unterschieben, um kuenftig mitzulesen.
...
-- Mitglieder sehen alle Schluesselzeilen ihrer Unterhaltung. Entschluesseln
-- kann jeder nur seine eigene; die fremden verraten lediglich, WER schon einen
-- Schluessel hat. Genau das braucht der Client, um fehlende nachzuziehen.
...
-- Bewusst keine UPDATE- und keine DELETE-Policy: eine einmal geschriebene
-- Schluesselzeile ist unveraenderlich. Niemand kann einem anderen Mitglied
-- nachtraeglich einen untergeschobenen Schluessel unterjubeln.
...
begin
  ...
  if other_user is null or other_user = me then
    raise exception 'Ungueltige Person';
  end if;
...
begin
  ...
  if t is null or char_length(t) > 60 then
    raise exception 'Gruppenname braucht 1 bis 60 Zeichen';
  end if;
  if coalesce(array_length(p_members, 1), 0) > 49 then
    raise exception 'Hoechstens 49 weitere Mitglieder';
  end if;
  ...
  insert into public.conversation_members (conversation_id, user_id, role)
  select conv, p.id, 'member'
    from public.profiles p
   where p.id = any(p_members) and p.id <> me
...
set search_path = public, pg_temp as $$
begin
  if not public.is_owner(p_conversation) then
    raise exception 'Nur die Gruppenleitung darf einladen' using errcode = '42501';
  end if;
  if not exists (
...
-- Neue Epoche erzwingen. Der Client verteilt danach einen frischen
-- Gruppenschluessel an die jetzigen Mitglieder.
...
declare
  e int;
begin
  if not public.is_member(p_conversation) then
    raise exception 'Kein Mitglied dieser Unterhaltung' using errcode = '42501';
  end if;

  update public.conversations
     set key_epoch = key_epoch + 1
   where id = p_conversation
  returning key_epoch into e;
...
-- ----------------------------------------------------------------- Realtime --
-- Realtime respektiert RLS: jeder Abonnent bekommt nur, was er lesen darf.
-- Uebertragen wird ohnehin nur Ciphertext.

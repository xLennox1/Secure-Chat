...
function generateCode() {
  ...
  while (out.length < CODE_LEN) {
    for (const b of crypto.randomBytes(CODE_LEN)) if (b < limit && out.length < CODE_LEN) out.push(ALPHABET[b % ALPHABET.length])
  }
  return out.join('').match(/.{1,4}/g).join('-')
}
const hashCode = (normalized) => crypto.createHmac('sha256', pepper).update(normalized).digest('hex')
const codes = Array.from({ length: count }, generateCode)
const supabase = createClient(url, key, { auth: { persistSession: false } })
const { error } = await supabase.from('invite_codes').insert(codes.map((code) => ({ kind:'invite', code_hash:hashCode(code.replaceAll('-','')), label, expires_at:expiresAt })))
if (error) { console.error('Einspielen fehlgeschlagen:', error.message); process.exit(1) }
console.log('\n' + count + ' Code' + (count === 1 ? '' : 's') + ' angelegt' + (label ? ' (' + label + ')' : '') + (expiresAt ? ', gueltig bis ' + new Date(expiresAt).toLocaleDateString('de-DE') : '') + ':\n')
